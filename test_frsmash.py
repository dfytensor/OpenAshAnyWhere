...
# CopyFirst 版本 (小 vocab)
class FRSMASH_CF(FRSMASH):
    def __init__(self):
        super().__init__(V, H, 4, 2, K=8)
...
torch.manual_seed(42); m=FRSMASH_CF().to(device)
n=sum(p.numel() for p in m.parameters())
print(f"FRSMASH-CF: {n:,} params")
...
# 对比 OpenASH (从之前的实验)
print(f"\nComparison:")
print(f"  FRSMASH:     best={best:.5f} params={n:,}")
print(f"  OpenASH:     best=1.5788 params=156,035 (CF failed)")
print(f"  HybridFRSM:  best=0.00026 params=395,745 (CF@65K=100%)")
print(f"  V6a-Fast:    best=0.00041 params=518,436 (CF@65K=100%)")
print("Done.")
