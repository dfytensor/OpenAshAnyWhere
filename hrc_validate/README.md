...
## Meta-ASH 30M 终审判决: SFT 损伤非繁衍噪声, 机制线关闭 (2026-09-16 minisft_test.py)
- 决定性测试: 同起点 (pt_full) 同 6k 步 SFT 同评测集
- **repro-on 3.8961 vs repro-off 4.0204 (+0.12, 更差)**; repro-off 训练损失同样游走 (3.60-4.08)
- 判据预设 (minisft_test.py 头部): 差异<0.15 或更差 => 收线. 触发.
- **最终结论链**: 1) 快验 -0.79 = 短程正则效应, 不可外推; 2) 全量 PT 打平 (~3.7 vs 3.86);
  3) 全量 SFT 不收敛 (3.63 vs 3.03), 且与繁衍项无关 (去掉更差);
  4) 门控单独有害 (4.31 vs 4.00); 5) Meta 机制在 30M 全量尺度无收益且有损害
- Meta-ASH 30M 线正式关闭. MetaRU v2 '门控在' + 本轮 '内稳态极化无害于 PT/毒于 SFT 且非主因'
  -- 机制组合的全部部件均已单独排查, 无幸存者
- 方法论沉淀: 快验增益必须过中程 (8k) 全管线条件复验 + 终点全量复验, 两关都过才能立案
