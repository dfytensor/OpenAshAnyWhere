"""
FRSM V6a MoE SFT Training Script
Supervised Fine-Tuning on conversation data (Sparse MoE)
"""
import os
import sys
import time
...
def train(config, pretrain_ckpt):
    print("=" * 60)
    print("FRSM V6a MoE SFT (Supervised Fine-Tuning)")
    print("=" * 60)
    ...
    latest_path = ckpt_dir / "frsm_v6_moe_sft_latest.pt"
    final_path = ckpt_dir / "frsm_v6_moe_sft_final.pt"
    ...
    print(f"\nStarting SFT ({len(train_loader.dataset)} samples, "
    ...
    while global_step < config.max_steps:
        ...
        if global_step % max(1, config.save_interval) == 0 and global_step > 0:
            tmp_path = ckpt_dir / "frsm_v6_moe_sft_latest.tmp"
            torch.save({
                'step': global_step,
...
def main():
    ...
    parser = argparse.ArgumentParser(description="FRSM V6a MoE SFT")
    ...
    parser.add_argument("--n_activated", type=int, default=4)
    parser.add_argument("--n_shared", type=int, default=1)
    parser.add_argument("--aux_loss_weight", type=float, default=0.01)
    parser.add_argument("--router_noise", type=float, default=0.0,
                        help="Router noise during SFT (default off)")
    ...
    parser.add_argument("--output_dir", type=str, default="frsm_v6_moe_sft_checkpoints")
    parser.add_argument("--data_dir", type=str, default="minimind_data")
    parser.add_argument("--pretrain_ckpt", type=str,
                        default="frsm_v6_moe_checkpoints/frsm_v6_moe_final.pt")
