"""
Needle-in-a-Haystack fine-tuning for 30M+cd model
Generate synthetic needle retrieval training data, then SFT on it.
"""
...
def gen_val(val_type):
    if val_type == "digit": return random.choice(DIGITS_POOL)
    if val_type == "digit3": return str(random.randint(100, 599))
    if val_type == "digit4": return str(random.randint(1000, 9999))
    if val_type == "date": return "{}年{}月{}日".format(random.randint(2020, 2026), random.randint(1, 12), random.randint(1, 28))
    if val_type == "card": return "6225880" + str(random.randint(10000000, 99999999))
    if val_type == "location": return random.choice(LOCATIONS)
    if val_type == "name_cat": return random.choice(CAT_NAMES)
    if val_type == "name_person": return random.choice(PERSON_NAMES)
    if val_type == "color": return random.choice(COLORS)
    if val_type == "city": return random.choice(CITIES)
    if val_type == "food": return random.choice(FOODS)
    if val_type == "plate": return random.choice(PLATES)
    if val_type == "english_name": return random.choice(ENGLISH_NAMES)
    if val_type == "door": return random.choice(DOORS)
    if val_type == "school": return random.choice(SCHOOLS)
    if val_type == "birthday": return random.choice(BIRTHDAYS)
    if val_type == "wifi": return random.choice(WIFIS)
    if val_type == "fruit": return random.choice(FRUITS)
    if val_type == "company": return random.choice(COMPANIES)
...
def train_needle(model, train_data, val_data, dev, vs):
    model.train()
    ...
    train_ds = NeedleDataset(train_data)
    val_ds = NeedleDataset(val_data)
    ...
    ckp_path = f"{OUT_DIR}/openash30m_cd_needle_latest.pth"
    global_step = 0
    best_loss = float('inf')
    ...
    print(f"[Needle SFT] {N_EPOCHS} epochs, {steps_per_epoch} steps/epoch, {total_steps} total")
    print(f"  LR={LR}, CAP={STATE_CAP}, DECAY={STATE_DECAY}, SEQ={SFT_SEQ}")
    ...
    for epoch in range(start_epoch, N_EPOCHS):
        ...
        for step_in_epoch in range(steps_per_epoch):
            ...
            for micro in range(GRAD_ACCUM):
                try:
                    ...
                    it = iter(loader)
                    x, t = next(it)
                x = x[:, :SFT_SEQ].to(dev, non_blocking=True)
                t = t[:, :SFT_SEQ].to(dev, non_blocking=True)
                x = x.clamp(0, vs - 1)
                t = t.clamp(0, vs - 1)
                with torch.amp.autocast('cuda', dtype=torch.bfloat16):
                    ...
                    loss = F.cross_entropy(logits.reshape(-1, vs), t.reshape(-1),
            ...
            if global_step % LOG_EVERY == 0:
                ...
                tok_s = global_step * GRAD_ACCUM * BATCH_SIZE * SFT_SEQ / elapsed
                print(f"  [needle] e{epoch + 1}/{N_EPOCHS} s{global_step:>6d}/{total_steps} "
                      f"loss={avg:.4f} lr={lr:.2e} {tok_s:.0f}tok/s", flush=True)
                running_loss = 0.0
                if avg < best_loss:
                    best_loss = avg
        ...
        print(f"  [needle] EPOCH {epoch + 1}/{N_EPOCHS} complete", flush=True)

    final_path = f"{OUT_DIR}/openash30m_cd_needle_final.pth"
    safe_save({'model': model.state_dict(), 'step': global_step, 'best_loss': best_loss}, final_path)
    print(f"[Needle SFT] Final: {final_path}")
    return model


@torch.no_grad()
def needle_eval(model, voc, sp, novel_chunks, dev, vs, n_trials=20):
    model.eval()
    n_layers = len(model.decoder_layers)
    NEEDLES = [
    ...
    print("\n" + "=" * 70)
    print("  Needle-in-a-Haystack Evaluation (post fine-tuning)")
    print("=" * 70)
    ...
    for cl in [512, 2048, 8192]:
        for d in [0, 50, 100]:
            ...
            for trial in range(n_trials):
                ...
                ctx_chunk = random.choice(novel_chunks)
                context_ids = voc.encode(ctx_chunk)
                max_ctx = cl - len(needle_ids) - len(question_ids) - 20
                context_ids = context_ids[:max(max_ctx, 20)]
                ...
                print("  {:>10} t{:>2} {} ans={:<6} resp={:.60}".format(
                    lb, trial + 1, mark, answer, resp_text.replace('\n', ' ')))
                sys.stdout.flush()

            acc = hits / n_trials
            lb = "{}K@{}%".format(cl // 1024 if cl >= 1024 else cl, d)
            print("  {:>10} ACC = {:.0%} ({}/{})\n".format(lb, acc, hits, n_trials))
...
if __name__ == "__main__":
    ...
    print("Loading novel for context...")
    novel_chunks = load_novel_chunks(novel_path, n_chunks=500, chunk_size=200)
    print(f"  {len(novel_chunks)} context chunks loaded")

    print(f"Generating {N_TRAIN} needle training samples...")
    train_data = []
    for i in range(N_TRAIN):
        train_data.append(generate_needle_sample(voc, sp, novel_chunks, SFT_SEQ))
        if (i + 1) % 5000 == 0:
            print(f"  ... {i + 1}/{N_TRAIN}", flush=True)
    print(f"  Done: {len(train_data)} train samples")

    print(f"Generating {N_VAL} needle validation samples...")
    val_data = [generate_needle_sample(voc, sp, novel_chunks, SFT_SEQ) for _ in range(N_VAL)]

    dev = torch.device('cuda:0')
    model = OpenASH(voc_size=vs, hidden_size=HIDDEN_SIZE, num_heads=NUM_HEADS,
                    num_layers=NUM_LAYERS, model_flag="train").to(dev)

    base_path = f"{OUT_DIR}/openash30m_cd_sft_final.pth"
    print(f"Loading base weights: {base_path}")
    model.load_state_dict(torch.load(base_path, map_location=dev)['model'])
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Model: {n_params:,} params")

    print(f"\n{'=' * 60}")
    print(f"  Needle SFT: {N_TRAIN} train, {N_VAL} val, {N_EPOCHS} epochs")
    print(f"  CAP={STATE_CAP}, DECAY={STATE_DECAY}, SEQ={SFT_SEQ}")
    print(f"{'=' * 60}")

    model = train_needle(model, train_data, val_data, dev, vs)

    print("\nRunning post-training needle evaluation...")
    needle_eval(model, voc, sp, novel_chunks, dev, vs, n_trials=20)
    print("\nDone!")
