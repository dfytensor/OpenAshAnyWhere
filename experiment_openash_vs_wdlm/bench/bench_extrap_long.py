#!/usr/bin/env python3
"""Extrapolation beyond training seq — concatenated samples"""
...
oa.load_state_dict(torch.load(os.path.join(BENCH, "openash60m_sft_final.pth"), map_location=DEV)["model"])
oa.to(DEV).eval()
...
# Collect all samples and concatenate into one long sequence
all_ids = []
...
def chunk_ppl(model, ids_list, sl, chunk=64):
    s = ids_list[:sl]
...
print()
print("=" * 62)
print("  Extrapolation: chunked PPL (chunk=64, concatenated data)")
print("  OA train seq=768 | WM train seq=1024")
print("=" * 62)
print(f"  {'Seq':>5}  {'OpenASH':>10}  {'WDLM':>10}  {'Delta':>8}  Note")
print(f"  {'-'*56}")

for sl in seqs:
    t0 = time.time()
    o = chunk_ppl(oa, all_ids, sl, CHUNK)
    w = chunk_ppl(wm, all_ids, sl, CHUNK)
    dt = time.time() - t0
    note = ""
    if sl == 768: note = "<-- OA train limit"
    elif sl == 1024: note = "<-- WM train limit"
    elif sl > 1024: note = "<-- BEYOND TRAINING"
    print(f"  {sl:>5}  {o:>10.2f}  {w:>10.2f}  {w-o:>+7.2f}  {note}")
    sys.stdout.flush()
