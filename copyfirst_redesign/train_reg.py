"""M1 训练: OpenASH 768x12 + FactRegister, 真实文本微调."""
import os, sys, time, random
sys.path.insert(0, r"F:\OpenASH2605")
sys.path.insert(0, r"F:\OpenASH2605\copyfirst_redesign")
import torch
import torch.nn.functional as F
from openash_reg import build_reg_model, MARK_OPEN, MARK_CLOSE, QUESTION
...
def main():
    ...
    print("数据:", len(seqs), "条", flush=True)

    model = build_reg_model(r"F:\OpenASH2605\models\full_sft_768_12.pth").to(DEV)
    n_train = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print("可训练参数: %.1fM" % (n_train / 1e6), flush=True)
    ...
    for step in range(STEPS):
        ...
        opt.step()
        if step % 250 == 0:
            acc = eval_acc(model, enc, seqs, [32, 128, 512])
            print("step %d loss=%.3f  acc(32/128/512)=%s (%.0fs)" % (
                step, loss.item(), "/".join("%.0f" % a for a in acc),
                time.time() - t0), flush=True)
    torch.save({"model": model.state_dict()},
               os.path.join(OUT, "openash_reg_sft.pth"))
...
def eval_acc(model, enc, seqs, gaps, n=16):
    ...
    for g in gaps:
        ...
        for _ in range(n):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                out, _ = model(t)
            pred = out[0, v_pos - 1].argmax().item()
            c += (pred == vid)
        accs.append(c / n * 100)
    return accs
