...
def main():
    ...
    for step in range(STEPS):
        ...
        for _ in range(BATCH):
            tokens, v_pos, vid, fmask = build_needle_sample(enc, seqs, max_len=384, n_fact=3)
            t = torch.tensor(tokens, dtype=torch.long)
            xs.append(t)
            y = t.clone(); y[:-1] = t[1:]; y[-1] = 0
            w = torch.zeros_like(y); w[v_pos - 1] = 5.0
            ys.append(y); ws.append(w)
            fm.append(torch.tensor(fmask, dtype=torch.float32))
        ...
        FM = torch.zeros(BATCH, L, device=DEV)
        for i, (x, y, w, fm_) in enumerate(zip(xs, ys, ws, fm)):
            X[i, :x.shape[0]] = x; Y[i, :y.shape[0]] = y; W[i, :w.shape[0]] = w
            FM[i, :fm_.shape[0]] = fm_
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            sparsity = torch.sigmoid(g_raw).mean()
            # 监督门: 事实 span 处应开 (BCE, 负样本加权抑制填充提交)
            pos_w = FM.sum() + 1
            neg_w = (1 - FM).sum() + 1
            gate_bce = F.binary_cross_entropy_with_logits(g_raw, FM, reduction="none")
            gate_bce = (gate_bce * (FM * neg_w / pos_w + (1 - FM))).mean()
            loss = ce + 0.02 * sparsity + 1.0 * gate_bce
        opt.zero_grad(set_to_none=True)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        opt.step()
        if step % 250 == 0:
            acc = eval_acc(model, enc, seqs)
            print("step %d loss=%.3f acc(1f64/1f512/2f512/3f512)=%s (%.0fs)" % (
...
def eval_acc(model, enc, seqs, n=24):
    model.eval()
    outs = []
    for nf, g in [(1, 64), (1, 512), (2, 512), (3, 512)]:
        c = 0
        for _ in range(n):
