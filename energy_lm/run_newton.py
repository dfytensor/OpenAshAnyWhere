"""Train the non-contractive (Newton-Krylov) DEQ on MiniMind-zh.

    python -m energy_lm.run_newton --n_map_layers 2 --steps 6000 --baseline
"""
...
from .noncontractive_trainer import NewtonDEQConfig, NewtonDEQTrainer
...
def get_args():
    ...
    p.add_argument("--newton_steps", type=int, default=4)
    p.add_argument("--newton_alpha", type=float, default=0.6)
    p.add_argument("--matrix_cap", type=float, default=3.0)
    ...
    p.add_argument("--out_dir", type=str, default="energy_lm/runs_newton")
    return p.parse_args()


@torch.no_grad()
def generate(model, tok, prompt, n_new=60, temperature=0.6, top_k=12):
    dev = model.tok_emb.device
    ids = torch.tensor([tok.encode(prompt)], dtype=torch.long, device=dev)
    max_len = model.cfg.max_seq_len
    from torch.func import jvp
    from energy_lm.deq_trainer import DEQTrainer
    gmres = DEQTrainer._gmres
    for _ in range(n_new):
        ctx = ids[:, -max_len:]
        X = model.embed(ctx)
        # Newton-Krylov forward (4 steps)
        f_only = lambda z: model.forward_diff(z, X)
        Z = X.clone()
        for _ in range(4):
            target = f_only(Z); gvec = target - Z
            if gvec.norm().item() / max(1, Z.numel()) < 1e-4: break
            def Av(w, _Z=Z):
                ws = w.view_as(_Z); _, jv = jvp(f_only, (_Z,), (ws,)); return (jv - ws).reshape(-1)
            try:
                delta = gmres(Av, (-gvec).reshape(-1), 8).view_as(Z)
            except Exception:
                break
            Z = (Z + 0.6 * delta).detach()
        logits = (Z @ model.output_weight + model.b_out)[0, -1] / max(temperature, 1e-5)
        if top_k > 0:
...
def main():
    ...
    print(f"[device] {device} | Newton-Krylov DEQ, map_layers={args.n_map_layers}")
    ...
    print(f"[model] DeepERB (Newton-Krylov) params: {count_params(model):,}")

    tcfg = NewtonDEQConfig(
        lr=args.lr, lr_out=args.lr_out, lr_emb=args.lr, gmres_k=args.gmres_k,
        newton_steps=args.newton_steps, newton_alpha=args.newton_alpha,
        matrix_cap=args.matrix_cap, total_steps=args.steps, warmup=args.warmup,
        device=str(device),
    )
    trainer = NewtonDEQTrainer(model, tcfg)
    ...
    print("\n=== final Newton-Krylov DEQ samples ===")
    for pr in PROMPTS:
    ...
    torch.save({"state_dict": model.state_dict(), "itos": tok.itos},
               os.path.join(out_dir, "newton_energylm.pt"))
