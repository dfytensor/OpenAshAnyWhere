...
class ScaleBlock(nn.Module):
    ...
    def forward(self, h, inp):
        c = torch.cat([h, inp], -1)
        f = torch.sigmoid(self.Wf(c))
        i = torch.sigmoid(self.Wi(c))
        return self.norm(f * h + i * torch.tanh(self.Wc(c)))

class V5a(nn.Module):
    def __init__(self, vs, H=128):
        super().__init__()
        self.H = H
        self.embed = nn.Embedding(vs, H)
        self.input_proj = nn.Linear(H, H)
        self.input_norm = nn.LayerNorm(H)
        # LM Path: 2 scales
        self.lm_scales = nn.ModuleList([ScaleBlock(H) for _ in range(2)])
        self.lm_proj = nn.ModuleList([nn.Linear(H, H) for _ in range(2)])
        self.lm_fusion = nn.Linear(H*2, H); self.lm_fusion_norm = nn.LayerNorm(H)
        # Memory Path: 4 scales
        self.mem_scales = nn.ModuleList([ScaleBlock(H) for _ in range(4)])
        self.mem_proj = nn.ModuleList([nn.Linear(H, H) for _ in range(4)])
        self.mem_fusion = nn.Linear(H*4, H); self.mem_fusion_norm = nn.LayerNorm(H)
        # Gate
    ...
    def forward(self, x, hp=None):
        ...
        outs = []
        for t in range(T):
            inp = self.input_norm(self.input_proj(xe[:, t, :]))
            # LM Path
            lm_nh = []
            for s in range(2):
                if t % (2**s) == 0:
                    lm_nh.append(self.lm_scales[s](lm_h[s], inp))
                else:
                    lm_nh.append(lm_h[s])
            lm_h = lm_nh
            lo = self.lm_fusion_norm(self.lm_fusion(torch.cat(lm_h, -1)) +
                                     sum(self.lm_proj[s](lm_h[s]) for s in range(2)) / 2)
            # Memory Path
            mem_nh = []
            for s in range(4):
                if t % (2**s) == 0:
                    mem_nh.append(self.mem_scales[s](mem_h[s], inp))
                else:
                    mem_nh.append(mem_h[s])
            mem_h = mem_nh
            mo = self.mem_fusion_norm(self.mem_fusion(torch.cat(mem_h, -1)) +
                                      sum(self.mem_proj[s](mem_h[s]) for s in range(4)) / 4)
            # Blend
            g = self.gate(inp)
...
def make_batch(bs, nl):
    t = torch.randint(2, V, (bs,))
    n = torch.randint(2, V, (bs, nl))
...
m = V5a(V, 128).to(device)
print(f"V5a params: {sum(p.numel() for p in m.parameters()):,}")
...
print(f"Converged: best={best:.5f}",flush=True)
